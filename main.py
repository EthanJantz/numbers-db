...
from status import TerminalSpinner
...
def main():
    ...
    with status.working(f"Contemplating {args.query}"):
        response = llm.prompt(prompt)
    num_description_msg = response["choices"][0]["message"]["content"] or ""

    with status.working("Consulting with the elders"):
    ...
    with status.working(f"Divining the essence of {args.query}"):
        response = llm.prompt(
            prompt,
